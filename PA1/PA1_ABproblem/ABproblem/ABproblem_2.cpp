...
int main()
{
	...
	int* A= new int[5000];
	int* B=new int[5000];
	int* C= new int[10000]();	//申请动态内存空间存放AB相乘的结果用于输出，为int数组；
	...
	{	
		...
		while(stra[lengtha]!='\0'){A[lengtha]=stra[lengtha]-'0';lengtha++;}	//将char型的数字转换为int型；
		while(strb[lengthb]!='\0'){B[lengthb]=strb[lengthb]-'0';lengthb++;}
